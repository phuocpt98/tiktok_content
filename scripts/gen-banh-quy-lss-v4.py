...
RATE_TRIES = ["+45%", "+55%"]   # v4: nói nhanh hơn, target ~16s
...
# ---- TIMELINE (v4: bỏ price, text ngắn, voice nhanh, ~16s) ----
...
SEGMENTS = [
    ...
    ("hook", 2.3,
     "Ơ, cái này gì mà ngon vậy trời?",
     "Ơ cái gì mà\nngon vậy trời?!",
     ["scene-001.mp4", "scene-002.mp4"]),
    ("intro", 4.5,
     "Bánh quy LSS 8 vị: socola, việt quất, nhân sữa chảy, gấu cute!",
     "BÁNH QUY LSS — 8 VỊ\nsocola • việt quất\nnhân sữa chảy • gấu cute",
     ["scene-004.mp4", "scene-005.mp4", "scene-015.mp4"]),
    ("experience", 2.8,
     "Mỗi miếng một bất ngờ — béo ngọt đủ cả!",
     "Mỗi miếng\nmột bất ngờ!",
     ["scene-007.mp4", "scene-010.mp4", "scene-013.mp4"]),
    ("follow", 3.2,
     "Thả tym và follow ủng hộ mình nha công chúa!",
     "Thả tym + follow\nnha công chúa ơi!",
     ["scene-011.mp4", "scene-014.mp4", "scene-017.mp4"]),
]

TOTAL = sum(s[1] for s in SEGMENTS)  # ~15s target
...
def main():
    ...
    # 4) MUX audio + video
    final = OUT_FINAL / "260422-banh-quy-lss-v4-fast-16s.mp4"
    run(["ffmpeg", "-y",
