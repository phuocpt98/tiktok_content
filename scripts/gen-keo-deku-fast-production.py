...
LABEL_Y = 280
SPEED_FACTOR = 1.75 # Tăng tốc x1.75

VIDEOS_DATA = [
    {
        "id": "v1_fixed",
        "caption": "Kẹo nén sữa chua Deku - Ăn là ghiền, màu pastel cưng xỉu 🔥 #keodeku #asmr #anvat #pelpel",
        "segments": [
    ...
    {
        "id": "v3_fixed",
        "caption": "SỮA CHUA hay SỬA CHUA? 99% người vẫn sai từ này 🧐 #keodeku #vuanhvien #chinhta #pelpel",
        "segments": [
    ...
    {
        "id": "v8_fixed",
        "caption": "Mẹ bảo lớn rồi còn ăn kẹo, và cái kết cực sốc sau 3 giây 🍬 #keodeku #funny #momlife #pelpel",
        "segments": [
...
async def build_single_video_fast(video_meta, all_scenes_meta):
    ...
    for i, seg in enumerate(video_meta["segments"], 1):
        ...
        generate_voice_fast(seg["text"], voice_path) # Voice đã được speedup
    ...
    file_name = video_meta['caption'].replace("🔥", "⚡️") + ".mp4"
    ...
    print(f"✅ Fast x1.75 Ready: {final_path.name}")

async def main():
    FINAL_DIR.mkdir(parents=True, exist_ok=True); all_scenes_meta = load_all_scenes()
    # Build 5 video mẫu tiêu biểu nhất với tốc độ x1.75
