...
TMP_DIR = OUTPUT_DIR / "_tmp_bait_quy_tac"
...
CAPTION = "3 Quy tắc ngầm khi mở Que Cay trong lớp - Số 3 là đáng sợ nhất 💀 #quecay #anvatvung #pelpel #funny #xuhuong"
FINAL_NAME = f"{CAPTION}.mp4"

SEGMENTS = [
    {
        "text": "3 Quy tắc ngầm khi mở bịch Que Cay ở trong lớp. Quy tắc số 3 là đáng sợ nhất đó nha!",
        "subtitle": "3 Quy tắc ngầm khi mở Que Cay trong lớp",
        "keywords": ["que cay"],
        "sfx": "clean_rustle.mp3",
        "sfx_vol": 0.8
    },
    {
        "text": "Một: Đừng bao giờ mở bịch quá to. Tiếng sột soạt sẽ tố cáo bạn với cả lớp ngay lập tức.",
        "subtitle": "1: Đừng mở bịch quá to!",
        "keywords": ["anvat"],
        "sfx": "clean_rustle.mp3",
        "sfx_vol": 1.0
    },
    {
        "text": "Hai: Tuyệt đối không để đứa bạn thân biết. Vì nó sẽ xử đẹp cả bịch trong 3 nốt nhạc.",
        "subtitle": "2: Tuyệt đối không để đứa bạn thân biết!",
        "keywords": ["thần long"],
        "sfx": "clean_crunch.mp3",
        "sfx_vol": 0.6
    },
    {
        "text": "Ba: Luôn thủ sẵn một bịch dự phòng dưới gầm bàn. Vì cơn thèm có thể ập tới bất cứ lúc nào!",
        "subtitle": "3: Luôn thủ sẵn bịch dự phòng!",
        "keywords": ["hằng đại"],
        "sfx": "clean_crunch.mp3",
        "sfx_vol": 0.5
    },
    {
        "text": "Tag ngay đứa bạn hay ăn vụng vào đây để cảnh cáo nó nào!",
        "subtitle": "Tag ngay đứa bạn hay ăn vụng vào đây!",
        "keywords": ["pelpel"],
        "sfx": None
    }
]
...
def build_segment_video(paths, dur, label_png, sub_png, out, i, total_segments):
    ...
    if num_paths > 1: base_filter = f"{inputs_str}concat=n={num_paths}:v=1:a=0[vcat];[vcat]"
    else: base_filter = "[0:v]"
    ...
    vf = f"{base_filter}[vpre][{label_idx}:v]overlay=(main_w-overlay_w)/2:{LABEL_Y}[v1];[v1][{sub_idx}:v]overlay=(main_w-overlay_w)/2:1580[v2];[v2]{final_filter}[vo]"
    cmd = ["ffmpeg", "-y"]
...
async def main():
    ...
    filter_complex = f"[0:a]volume=0.2[bg];[1:a]volume=1.4[v];"
    mix_inputs = f"[bg][v]"
    sfx_args = []
    ...
    print(f"\n✅ Video Quy tắc Ready: {final_path.relative_to(ROOT)}")
