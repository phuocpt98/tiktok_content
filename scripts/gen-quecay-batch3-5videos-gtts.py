"""
Generate Batch 3 (5 videos) using gTTS (Google chị Google) for maximum stability and correctness.
- 2 Vua Tiếng Việt scripts
- 3 Review scripts
- Robust TTS: Try gTTS first (legendary Google Voice), then edge-tts, then Gemini.
"""
import json
import os
import subprocess
...
# TTS Settings
VOICES_EDGE = ["vi-VN-NamMinhNeural", "vi-VN-HoaiMyNeural"]
TTS_RATE_EDGE = "+10%"
...
def clean_text_for_tts(text: str) -> str:
    # Hỗ trợ đầy đủ tiếng Việt VIẾT HOA và VIẾT THƯỜNG
...
async def tts_gtts(text: str, out_mp3: Path):
    """Sử dụng giọng đọc chị Google huyền thoại."""
    try:
        print(f"  → Using gTTS for: {text[:20]}...")
        tts = gTTS(text=text, lang='vi')
        tts.save(str(out_mp3))
        if out_mp3.exists() and out_mp3.stat().st_size > 500:
            # ADD 0.2s silence padding
...
async def tts_robust(text: str, out_mp3: Path):
    ...
    # Ưu tiên gTTS đầu tiên
    if await tts_gtts(cleaned, out_mp3): return
    
    # Fallback Edge TTS
    for voice in VOICES_EDGE:
        try:
            c = edge_tts.Communicate(cleaned, voice, rate=TTS_RATE_EDGE)
    ...
    if gemini_client:
        try:
            response = gemini_client.models.generate_content(model="gemini-2.0-flash", contents=f"Đọc văn bản sau: {cleaned}")
...
async def build_video(config: dict):
    tmp = OUTPUT_DIR / f"_tmp_gtts_{config['name']}"
...
async def main():
    ...
    for cfg in VIDEOS_CONFIG:
        print(f"Building {cfg['name']} with gTTS...")
