...
TMP_DIR = OUTPUT_DIR / "_tmp_v3_clean"
...
CAPTION = "3 giờ sáng và cơn nghiện Que Cay trỗi dậy 🔥 #quecay #anvat #pelpel #asmr #midnightcravings"
FINAL_NAME = f"{CAPTION}.mp4"

SEGMENTS = [
    {
        "text_voice": "3 giờ sáng rồi mà chiếc bụng đói cứ gào thét tên... QUE CAY!",
        "text_subtitle": "3 giờ sáng và cơn nghiện QUE CAY!",
        "keywords": ["que cay"],
        "sfx": "clean_rustle.mp3", # Tiếng sột soạt mở đầu
        "sfx_vol": 0.5
    },
    {
        "text_voice": "Nhìn sớ thịt dai dai thấm đẫm sốt cay nồng này xem, ai mà chịu cho nổi?",
        "text_subtitle": "Sớ thịt dai dai — Thấm đẫm sốt cay",
        "keywords": ["thần long"],
        "sfx": "clean_crunch.mp3", # Tiếng nhai giòn
        "sfx_vol": 0.7
    },
    {
        "text_voice": "Hàng Đại hay Vương Thần Long cũng đều là chân ái đêm khuya hết nha.",
        "text_subtitle": "Hàng Đại hay Vương Thần Long đều đỉnh!",
        "keywords": ["hằng đại"],
        "sfx": "clean_crunch.mp3",
        "sfx_vol": 0.4
    },
    {
        "text_voice": "Lỡ va phải Pel Pel rồi thì chốt đơn ngay đi chứ đợi gì nữa cưng ơi!",
        "text_subtitle": "Va phải Pel Pel rồi — Chốt đơn ngay!",
        "keywords": ["anvat"],
        "sfx": None
    }
]
...
def process_clip_clean(src, dur, label_png, sub_png, voice_mp3, sfx_name, sfx_vol, out):
    ...
    # Render video (Mute tiếng gốc của scene)
    ...
    # Mix Voice + Clean SFX (Tiếng Voice Pel Pel đè lên SFX sạch)
    if sfx_name:
        sfx_path = SFX_DIR / sfx_name
        # Loop SFX if shorter than voice
        run(["ffmpeg", "-y", "-i", str(voice_mp3), "-stream_loop", "-1", "-i", str(sfx_path), "-filter_complex", f"[0:a]volume=1.2[v];[1:a]volume={sfx_vol}[s];[v][s]amix=inputs=2:duration=first[a]", "-map", "[a]", "-c:a", "aac", str(out.with_suffix(".m4a"))])
    else:
...
async def main():
    ...
    for i, seg in enumerate(SEGMENTS, 1):
        ...
        process_clip_clean(scenes[i % len(scenes)], dur, label_png, sub_png, voice_path, seg["sfx"], seg.get("sfx_vol", 0.5), clip_path)
        clip_paths.append(clip_path)
        print(f"Segment {i} Clean ASMR done")
    ...
    print(f"\n✅ Video Clean ASMR Ready: {final_path.relative_to(ROOT)}")
